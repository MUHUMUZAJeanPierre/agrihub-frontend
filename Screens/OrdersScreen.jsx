...
//     } else {
//       console.error('Error response:', data);  // Debug log for the error response
...
//           setOrders(data);
//           const pendingCount = data.filter(order => ['pending', 'processing', 'shipped'].includes(order.status?.toLowerCase())).length;
//           setPendingOrdersCount(pendingCount);
//         } else {
...
//     padding: 12,
//     borderWidth: 1,
//     borderColor: 'rgba(46, 125, 49, 0.1)',
//   },
//   itemsPreviewLabel: {
//     fontSize: 12,
//     fontWeight: '600',
...
const fetchOrders = async () => {
  ...
  // Updated filter function to handle the new requirement
  const filterOrders = () => {
    if (!orders || !Array.isArray(orders)) return [];

    if (activeTab === 'current') {
      // Current tab shows only Pending orders
      return orders.filter(order => {
        const status = order.status?.toLowerCase() || 'pending';
